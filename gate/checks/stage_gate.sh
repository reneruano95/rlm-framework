#!/bin/bash
# Runs as root: stage the gate's runtime scripts, extension, checks, artifacts and the
# scratchpad drivers. The spike user cannot read /mnt/d by design, so every copy out
# of the repo happens here.
set -e
...
mkdir -p /home/spike/gate/checks /home/spike/gate/artifacts /opt/spike-scripts /home/spike/prime-spike/extensions
...
for f in "$G"/artifacts/*; do
  [ -f "$f" ] && tr -d '\r' < "$f" > "/home/spike/gate/artifacts/$(basename "$f")"
done
...
echo "gate/artifacts/: $(ls /home/spike/gate/artifacts | tr '\n' ' ')"
