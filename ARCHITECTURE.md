...
**Spec version:** `rlm-runtime-spec-v0.1.1` (changelog: §14)
...
5. **Build-flag fine-tuning** (FA-path variants — plain HIP FA vs rocWMMA — and per-backend flags) — metric: raw prefill t/s. Residual after the S0 backend split; done last because it is fiddly and orthogonal.
...
- **A2 — Host OS. DECISION REQUIRED BEFORE S0 — the tripwire has fired: this spec is being authored on a Windows 11 machine.** The choice is convenience vs headroom + comparability, not possible vs impossible (v0.1.1 correction: ROCm is officially on Windows for gfx1151 since HIP SDK 7.1.1, Feb 2026).
  **Linux path:** rlimits + network namespace (C1); dynamic GTT ~110–120 GB (§4 host config); Mesa RADV for the root; and — decisive for §7/§8 — every community benchmark corpus behind this spec's priors is Linux, so on-box numbers stay comparable to reference data.
