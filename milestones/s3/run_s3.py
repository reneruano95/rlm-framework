...
- An interrupted gate must never cost the attempts that already ran:
  one JSONL record per check appended to milestones/s3/results/runs.jsonl; the report
...
S3_DIR = REPO_ROOT / "milestones" / "s3"
...
# --------------------------------------------------------------------------- #
# record helpers (milestones/s1/run_s1.py conventions)
# --------------------------------------------------------------------------- #
...
def render_report(runs: list[dict]) -> str:
    ...
    lines = [
        ...
        f"by `milestones/s3/run_s3.py --phase report` from `milestones/s3/results/runs.jsonl` and the "
        f"trace store `milestones/s3/results/store/`. Numbers come from the store, not "
        f"process memory._",
        "",
