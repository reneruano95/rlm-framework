...
**Runner:** `milestones/s2/run_cache_instrument.py` · **Tables:** `milestones/s2/analyse_cache_instrument.py` · **Raw:** `milestones/s2/results/cache_instrument.jsonl`
...
against 3 tokens of genuinely shared prefix (`milestones/s2/DISTANCE.md` §5). **The accusation
...
`milestones/s2/OCCUPANCY.md` measured as the entire latency-versus-occupancy effect. Scored
...
The last row is `milestones/s2/DISTANCE.md`'s anomaly, reproduced on demand: **958 reported
...
1. **`--cache-ram 0` (or `--no-cache-idle-slots`) must be in the launch line.**
   ...
   function of the prompts and not reproducible run to run**. `milestones/s2/OCCUPANCY.md`
...
2. **`ub + 4` is a property of a build and a flag (R11).** It was measured at 512
   ...
   changes. `milestones/s2/run_cache_instrument.py --condition <name> --extra "--cache-ram 0
...
2. **`--cache-ram 0` earns a second, independent justification.** `milestones/s2/OCCUPANCY.md`
