"""Drive milestones/s2/mtp_bench.py across DFlash2 / MTP / no-speculation arms.
...
(milestones/s2/mtp_bench.py) is NOT modified -- acceptance is recovered from the
...
milestones/s2/results/ came from the pinned b10375 clang release binary; this is an
...
LOGDIR = PROJECT / "milestones" / "s2" / "results" / "dflash2-logs"
PORT = 8080
        ...
        try:
            if not wait_health(timeout_s):
                ...
                return {"label": label, "ok": False, "error": "health timeout",
                        "log_tail": tail}
            print(f"  server up; running milestones/s2/mtp_bench.py --label {label}")
            r = subprocess.run(
                ["uv", "run", "--python", "3.12", "--no-project",
                 "milestones/s2/mtp_bench.py", "--label", label, "--reps", str(reps),
                 "--temp", str(temp)],
