...
against `milestones/s2/make_sweep_fixtures.py`.

`milestones/s2/REFUSAL-AB.md` §2 recorded an unplanned result with a bigger claim than the
...
factor with two levels at every size:
  ...
  * **matched-density** — every size carries the SAME number of distractor
    ...
    640-token cells (`milestones/s2/fixtures-refusal-640-s*/s2-640-p50.chunk.txt`, 3
...
if str(REPO_ROOT) not in sys.path:  # `uv run milestones/s2/make_distance_fixtures.py`
...
#: repeated prefix costs another ~311 tokens), which `milestones/s2/run_distance.py`
...
#: actually ran rather than chosen: `milestones/s2/fixtures-refusal-640-s*` carry 3 `ENT-`
#: bindings per 640-token chunk, `milestones/s2/fixtures-refusal-s*` 6 per 1,024 and
#: `milestones/s2/fixtures/s2-2048-p50.chunk.txt` 11 per 2,048 — one binding per ~186
...
def write_fixtures(count: Callable[[str], int], *, sizes: tuple[int, ...] = SIZES,
    ...
    One seed per directory, mirroring `milestones/s2/fixtures-refusal-*`: the runner pools
    ...
    manifest = {
        "generator": "milestones/s2/make_distance_fixtures.py",
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--offline", action="store_true",
                        help="build with the 4-chars-per-token proxy. Smoke-test "
                             ...
                             "milestones/s2/run_distance.py refuses it.")
