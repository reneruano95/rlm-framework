...
out = io.open("milestones/s2/audit/_misc_out.txt", "w", encoding="utf-8", errors="replace")
...
txt = io.open("milestones/s2/ARCH-LADDER.md", encoding="utf-8", errors="replace").read()
...
P("  .ps1 files in milestones/s2/: " + str([f for f in os.listdir("s2") if f.endswith(".ps1")]))
r = subprocess.run(["git","show","70e2d40:milestones/s2/arch_ladder.py"], capture_output=True, text=True, encoding="utf-8", errors="replace")
...
t = io.open("milestones/s2/R13-mitigations.md", encoding="utf-8", errors="replace").read().split("\n")
for i, l in enumerate(t, 1):
    if "cache_n" in l: P(f"  :{i}: {l.strip()[:220]}")

P("\n-- R13-slotcount np values --")
t = io.open("milestones/s2/R13-slotcount.md", encoding="utf-8", errors="replace").read().split("\n")
...
t = io.open("milestones/s2/CACHE-INSTRUMENT.md", encoding="utf-8", errors="replace").read().split("\n")
