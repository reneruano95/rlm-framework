...
THE BLOCKER THIS EXISTS TO CLEAR (`milestones/s2/DISTANCE.md` §5, §10 R8, §7 #2's
...
`milestones/s2/OCCUPANCY.md` measured as the entire latency-vs-occupancy effect. If that
...
if str(REPO_ROOT) not in sys.path:  # `uv run milestones/s2/run_cache_instrument.py`
...
def case_script(rep: int) -> list[Step]:
    ...
    steps: list[Step] = [
        ...
        # `milestones/s2/DISTANCE.md` §5's layout-C row, reconstructed: the document was
