...
THE NUMBER THIS EXISTS TO ATTACK. `milestones/s2/RESULTS.md` finding 3: asked about a fact
...
THE MEASUREMENT'S OWN RULES.
  ...
  * **No retries.** `milestones/s2/leafcall.py` deliberately has none, and adding one here
...
**2,560 tokens per slot** (`milestones/s2/R13-slotcount.md`), which is `chunk 1024 +
...
if str(REPO_ROOT) not in sys.path:  # `uv run milestones/s2/run_refusal_ab.py`
...
def supplied_identifier(text: str) -> bool:
    ...
    `milestones/s2/RESULTS.md` was scored with, and moving it would move the 95% baseline
...
def score(raw: str, *, envelope: bool, chunk: str | None, question_type: str,
    ...
    instrument `milestones/s2/RESULTS.md` was scored by."""
...
def rescore(records: list[dict]) -> list[dict]:
    ...
    `milestones/s2/run_sweep.py --phase report` does: a classifier fix must cost a re-read
...
def render_report(records: list[dict]) -> str:
    ...
    lines = [
        ...
        "",
        f"*Generated by `milestones/s2/run_refusal_ab.py --phase report` from "
        ...
        "",
        "Taxonomy is `milestones/s2/RESULTS.md`'s, unchanged, so the numbers are comparable: "
    ...
    lines += [
        ...
        "instrument `milestones/s2/RESULTS.md` was scored with, and moving it moves the "
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--report-md", default=None,
                        help="where to render the report (default "
                             "milestones/s2/REFUSAL-AB.md). The secondary chunk-size run "
