...
def render_report(verdict: Verdict, scorecard: Scorecard | None,
    ...
    #: v1's ordering (RLM, then B1-B3) is a module default; a v2 verdict
    #: carries its own `rules.arms` (e.g. RLM then rlm-nosubcalls/B2), and
    #: ordering the tables by the wrong arm list is harmless for v1 (its
    #: baselines and `ARMS` agree) but wrong for any manifest whose
    #: baselines differ from `("b1", "b2", "b3")`.
    rule_order = final.rules.arms if final.rules is not None else ARMS
    arms = [a for a in rule_order if a in final.arms] + \
           [a for a in final.arms if a not in rule_order]
    baselines = final.rules.baselines if final.rules is not None else BASELINES
    L: list[str] = []
    ...
    L += ["## Cost scorecard", "",
          ...
          "median energy J | median avg power W | "
          + " | ".join(f"×wall vs {b.upper()}" for b in baselines) + " |",
          "|---" * (7 + len(baselines)) + "|"]
    if scorecard is None:
        L.append("| _no scorecard supplied_ |" + " |" * (6 + len(baselines)))
    else:
        for arm in arms:
