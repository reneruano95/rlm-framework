...
scoring. So this module imports duckdb, `rlm.stats` and the standard library,
takes the manifest as an OBJECT (`bench/` is not in the shipped wheel, exactly
as `rlm/bench.py` documents), and reaches nothing that speaks HTTP.
...
def pareto_svg(verdict: Verdict, scorecard: Scorecard | None, *,
    ...
    only this repo, which is the same standard `rlm/stats.py` holds the
    p-values to.
    """
...
def render_report(verdict: Verdict, scorecard: Scorecard | None,
    ...
    L += [f"# S4 verdict — run `{verdict.run_id}`", "",
          "Generated by `rlm.verdict` from the closed trace store. Every "
          "figure below is recomputable from the record: the sign test and "
          "bootstrap are exact and dependency-free (`rlm/stats.py`), and the "
