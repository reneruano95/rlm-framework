...
Run this against each model in turn (one server at a time, same script):
    uv run --python 3.12 --no-project s2/arch_ladder.py --label qwen --policy virgin
    uv run --python 3.12 --no-project s2/arch_ladder.py --label qwen --policy shared
...
def main() -> None:
    ...
    out = Path(a.out or f"s2/results/arch_ladder_{a.label}_{a.policy}.jsonl")
