# s2/CACHE-INSTRUMENT.md's condition table, one server launch per condition.
...
#
# `--extra=$Extra`, never `--extra $Extra` (see s2/occupancy_conditions.ps1).
...
function Run-Cond {
  ...
  Write-Host "==== $Name ===="
  uv run --python 3.12 s2/run_cache_instrument.py --condition $Name --extra="$Extra" `
...
# Two independent ways to switch the host cache off (s2/OCCUPANCY.md).
