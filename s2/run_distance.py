...
window and 0/21 at 640 (`s2/REFUSAL-AB.md` §2, Fisher p = 8.7e-15). If the cause
...
THE DESIGN, three factors, one un-edited prompt.
  ...
  3 / 6 / 11). See `s2/make_distance_fixtures.py` for what a "binding" is and
  why it is that.
...
`s2/leafcall.py`), no prompt edits, no arm added after the first call, and no
scoring instrument of its own: labels come from `s2.run_sweep.classify`, the
same function `s2/RESULTS.md` and `s2/REFUSAL-AB.md` were scored with, so the
...
if str(REPO_ROOT) not in sys.path:  # `uv run s2/run_distance.py`
...
def score(raw: str, *, question_type: str, expected: str | None,
    ...
    `supplied_identifier` is imported from `s2/run_refusal_ab.py` for the reason
...
def render_report(records: list[dict]) -> str:
    ...
    lines = [
        ...
        "",
        f"*Generated by `s2/run_distance.py --phase report` from "
        f"`s2/results/distance.jsonl` ({len(records)} records) on "
        f"{datetime.now(timezone.utc).isoformat(timespec='seconds')}. Labels are "
        f"RE-DERIVED from each record's verbatim `raw_output` by "
        f"`s2.run_sweep.classify` — the instrument `s2/RESULTS.md` and "
        f"`s2/REFUSAL-AB.md` were scored with — not read back from the label "
    ...
    if 640 in rep_fp and 1024 in rep_fp:
        ...
        lines += [
            ...
            f"Fisher two-sided p = {p:.3g}.** `s2/REFUSAL-AB.md` recorded 30/30 "
