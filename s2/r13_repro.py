...
reads the corpus in `s2/fixtures/` as DATA and drives it with this file's
...
p = 4.4e-9. See `s2/R13.md`.

USAGE
  # the decisive paired run
  uv run --python 3.12 python s2/r13_repro.py --port 8081 \
      --replay-fixtures s2/fixtures --replay-trials 3 --paired-virgin \
      --temperature 0.3 --n-predict 512 --slot 0 --out s2/results/r13.jsonl

  # does an explicit slot erase clear it? (server needs --slot-save-path)
  uv run --python 3.12 python s2/r13_repro.py --port 8081 \
      --replay-fixtures s2/fixtures --erase-between ...

  # the synthetic design, and the size search that found no floor
  uv run --python 3.12 python s2/r13_repro.py --port 8081 --trials 5
  uv run --python 3.12 python s2/r13_repro.py --port 8081 \
  ...
  uv run --python 3.12 python s2/r13_repro.py --port 8082 --arch-report
"""
...
def main(argv: list[str] | None = None) -> int:
    ...
    ap.add_argument("--out", default="s2/results/r13_repro.jsonl")
    ...
    if args.slot_ab:
        ...
        run_slot_ab(
            srv,
            calls,
            Path(args.replay_fixtures or "s2/fixtures"),
            cell_a=args.cell_a,
            cell_b=args.cell_b,
            prime=args.prime,
            seeds=args.trials,
