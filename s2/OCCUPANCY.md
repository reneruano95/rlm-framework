...
**Runner:** `s2/run_occupancy.py` · **conditions:** `s2/occupancy_conditions.ps1`,
`s2/occupancy_rest.ps1`, `s2/occupancy_conc.ps1`, `s2/occupancy_warm.ps1` ·
`s2/occupancy_ladder.ps1` · **analysis:** `s2/analyse_occupancy.py` ·
**raw:** `s2/results/occupancy.jsonl`
...
`s2/DISTANCE.md` §4b reported per-call wall growing **1.15 s → 5.02 s** with
...
4. **Treat every wall-clock number recorded on this box before 2026-08-14 as
   ...
   occupancy is known. That includes every per-arm wall in `s2/DISTANCE.md`
   (which says so), `s2/REFUSAL-AB.md`, and the `-np` ladder in
   `s2/R13-slotcount.md` — the last of which priced `-np 128` on memory and
