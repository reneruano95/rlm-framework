...
to depth ten. Cells come from the seven named `s2/fixtures-refusal-640-s*` dirs,
...
Run against the SHIPPED leaf launch line (which carries --cache-ram 0):
    uv run --python 3.12 --no-project s2/gate_ab.py
"""
...
OUT = REPO / "s2" / "results" / "gate_ab.jsonl"
PROBE = "PROBEMARKER"

FIXTURE_DIRS = sorted((REPO / "s2").glob("fixtures-refusal-640-s*"))
