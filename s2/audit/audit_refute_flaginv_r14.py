...
recs=[json.loads(l) for l in open("s2/results/r14.jsonl",encoding="utf-8") if l.strip()]
...
# OCCUPANCY same
recs=[json.loads(l) for l in open("s2/results/occupancy.jsonl",encoding="utf-8") if l.strip()]
...
rs=[json.loads(l) for l in open("s2/results/r13_mitigation.jsonl",encoding="utf-8") if l.strip()]
print("r13_mitigation.jsonl n=", len(rs))
import glob
print("r13_mit_*.json count:", len(glob.glob("s2/results/r13_mit_*.json")))
