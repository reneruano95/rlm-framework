...
Measured 2026-08-13 with `s2/r13_mitigation_bench.py` on build `b10375-ba360efe1`
(ROCm), leaf `Qwen3.6-35B-A3B-UD-Q4_K_M`. The leak oracle (`foreign_strings`)
and the transport are imported unchanged from `s2/r13_repro.py`, so every
leak verdict below is produced by the same code that produced `s2/R13.md`.
...
| 5 | `action=erase` / save-restore | **NO** (33/54, `s2/R13.md` §2) | — | — | — | — |
| ★ | **one virgin SLOT per window, `-np 128`, k=8** | **YES — 0/72** | 948 | 54.9 → 10 | **0.05 s** (amortised restart) | **53.7 s** |

Raw records: `s2/results/r13_mitigation.jsonl` and `s2/results/r13_mit_*.json`.
...
`s2/results/r13_mit_nocacheprompt_full.json`, `r13_mit_posctl_full.json`.
...
This is the same conclusion `s2/R13.md` §2 row 2 reached from cold re-prefills,
...
`s2/results/r13_mit_pattern.json`. Same slot, same three questions, `cache_prompt`
the only variable.
...
> `s2/results/r13_mit_np1_shared.json`
...
`s2/results/r13_mit_tput_np64.json`. End to end over a 20×2 episode the same
...
Measured end to end (`s2/results/r13_mit_episode96_freshproc.json`, a real
...
`s2/R13.md` §8 identified the shape of this. This section prices it and finds it
...
the hybrid's 2 of 6 (`s2/R13.md` §3). Swapping to it would pay a different
...
(33/54 with erase vs 34/54 without, `s2/R13.md` §2 row 5). `save`/`restore` were
...
(Correction already recorded in `s2/R13.md` §6: `erase` is not 501 on b10375 —
...
decoded tokens. `s2/results/r13_mit_episode96_*.json`.
...
(`s2/results/r13_mit_tput_np1.json`, `r13_mit_np1_shared.json`).
...
3. **`fallback_leaf` is not an R13 mitigation** and should not be listed as one
   (falsified in `s2/R13.md` §3; not re-measured here, by design).
...
* `s2/r13_mitigation_bench.py` — the harness (`--mode pattern|throughput|windows|episode`).
* `s2/results/r13_mitigation.jsonl` — every call, with verdicts.
* `s2/results/r13_mit_posctl_full.json` — positive control, 15/18.
* `s2/results/r13_mit_nocacheprompt_full.json` — option 1, 15/18.
* `s2/results/r13_mit_np1_shared.json` — option 3, 4/18.
* `s2/results/r13_mit_smallladder.json`, `r13_mit_windows_np8.json` — size vs growth.
* `s2/results/r13_mit_virginscale_{1,7,13,19}.json`, `r13_mit_virgin_np128.json` — 0/72, 0/12.
* `s2/results/r13_mit_tput_np{1,8,64,128}*.json` — throughput curves.
* `s2/results/r13_mit_episode96_*.json`, `r13_mit_episode_*.json` — the episode table.
* `s2/results/r13_mit_pattern.json` — one chunk, three questions.
...
uv run --python 3.12 python s2/r13_mitigation_bench.py --mode windows \
