...
`s2/CARRYOVER.md` measured depth TWO — one priming question, then the measured
...
an 8-stem pool, and every result in `s2/results/arch_ladder_*.jsonl` is keyed to
...
Run at production geometry (needs ~90 slots for 8 fixtures at depth 10):
    ...
    uv run --python 3.12 --no-project s2/carryover_depth.py --label prod
"""
...
def main() -> None:
    ...
    out = Path(a.out or f"s2/results/carryover_depth_{a.label}.jsonl")
