...
R13 reproducer + upstream issue (privacy defect in the multi-user story); R14 `--no-cont-batching` reproducer (fan-out economics matter 10× more once delegation is real); the leaf horizon — v0.3.5 shows it is not a slot-state or prefix-reuse effect, and both models tested show the coarse 640-vs-1,024 step (gemma-4 as a positive control: its SWA window is exactly 1,024), but the rest of the serving path is untested, so "prompt-side" is a hypothesis rather than a finding, and a self-improving loop would be the natural place to search for it.
