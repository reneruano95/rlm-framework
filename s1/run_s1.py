...
record per attempt to `s1/results/runs.jsonl`, from which `--phase report`
regenerates `s1/RESULTS.md`. The phases are separable on purpose: an
...
if str(REPO_ROOT) not in sys.path:  # `uv run s1/run_s1.py` from the repo root
...
def render_report(runs: list[dict]) -> str:
    ...
    lines += [
        ...
        "",
        f"*Generated by `s1/run_s1.py --phase report` from `s1/results/runs.jsonl` "
